...
object RemoteModule {
    ...
    @Provides
    @Named("AUTHS")
    fun provideAuthsRetrofit(
        httpClient: OkHttpClient,
        gson: Gson
    ): Retrofit {
        return Retrofit.Builder().apply {
            client(httpClient)
            baseUrl(AppNetwork.BASE_AUTH)
            addConverterFactory(GsonConverterFactory.create(gson))
        }.build()
    }
    ...
    @Provides
    @Named("MEALS")
    fun provideMealsApi(@Named("MEALS") retrofit: Retrofit): RecipesApi {
    ...
    @Provides
    @Named("AUTHS")
    fun provideAuthsApi(@Named("AUTHS") retrofit: Retrofit): AuthsApi {
        return retrofit.create(AuthsApi::class.java)
    }
}